...
def _init_state():
    ...
    # ── Journey progress flags (additive — do not affect existing logic) ──
    st.session_state.setdefault("journey_portfolio_done",     False)
    st.session_state.setdefault("journey_optimization_done",  False)
    st.session_state.setdefault("journey_ai_review_done",     False)
    st.session_state.setdefault("journey_report_ready",       False)
    st.session_state.setdefault("_report_open",               False)
    st.session_state.setdefault("journey_skip_to_report",     False)
...
def render_product_selector():
    ...
    _optimizer_btns = [
        ("c1", "📊 ניתוח תיק לקוח", "תיק לקוח",        "pt_client"),
...
_LANDING_CSS = """
<style>
/* Profit Financial Group · Alpha Optimizer · Landing */
...
.tb{
  display:flex;align-items:center;justify-content:space-between;
  padding:0 32px;height:70px;background:#0b1929;
  border-bottom:1px solid rgba(255,255,255,.06);
  position:relative;
}
.tb::after{
  content:'';position:absolute;bottom:0;left:0;right:0;height:1px;
  background:linear-gradient(90deg,transparent,rgba(58,122,254,.35),transparent);
}
.tb-left{display:flex;align-items:center;gap:12px}
.tb-mark{
  width:38px;height:38px;border-radius:10px;
  background:linear-gradient(145deg,#1a3a6e,#2563eb);
  display:flex;align-items:center;justify-content:center;
  font-size:19px;flex-shrink:0;
  box-shadow:0 2px 10px rgba(37,99,235,.35);
}
.tb-names{display:flex;flex-direction:column;gap:2px}
.tb-co{
  font-size:17px;font-weight:900;color:#f1f5f9;
  letter-spacing:-.5px;line-height:1.1;
}
.tb-prod{
  font-size:11px;font-weight:600;color:rgba(96,165,250,.75);
  letter-spacing:.3px;
}
.tb-right{display:flex;align-items:center;gap:10px}
.tb-divider{width:1px;height:18px;background:rgba(255,255,255,.1)}
.tb-beta{
  border:1px solid rgba(250,204,21,.22);border-radius:999px;
  padding:2px 9px;font-size:9px;font-weight:700;
  color:rgba(251,191,36,.6);letter-spacing:1px;text-transform:uppercase;
}
.tb-tagline{
  font-size:9px;color:rgba(148,197,253,.3);
  letter-spacing:1.2px;text-transform:uppercase;
}

/* ── PAGE TITLE ─────────────────────────────────────────── */
.pt{padding:36px 0 6px;direction:rtl;text-align:right}
.pt-h{font-size:34px;font-weight:900;color:#0b1929;letter-spacing:-1.3px;margin:0 0 8px}
.pt-s{font-size:14px;color:#64748b;margin:0;font-weight:400}

/* ── CARD ───────────────────────────────────────────────── */
.card{
  background:#fff;border-radius:18px;padding:26px 22px 22px;
  border:1px solid #e8eef6;direction:rtl;text-align:right;
  display:flex;flex-direction:column;height:100%;
  box-shadow:0 2px 8px rgba(11,25,41,.05),0 1px 2px rgba(11,25,41,.04);
  transition:transform .2s,box-shadow .2s,border-color .2s;
  position:relative;overflow:hidden;
}
.card:hover{
  transform:translateY(-3px);
  box-shadow:0 12px 32px rgba(11,25,41,.1),0 3px 6px rgba(11,25,41,.05);
  border-color:#d0e2f7;
}
.card-primary{
  border-color:#dbeafe;
  background:linear-gradient(155deg,#f5f9ff 0%,#fff 55%);
}
.card-rule{
  position:absolute;top:0;left:0;right:0;height:3px;
  background:var(--rule,#94a3b8);border-radius:18px 18px 0 0;
}
.card-badge{
  font-size:9.5px;font-weight:700;letter-spacing:2px;text-transform:uppercase;
  color:var(--badge-c,#64748b);margin-bottom:14px;opacity:.8;
}
.card-ico{
  width:48px;height:48px;border-radius:13px;
  display:flex;align-items:center;justify-content:center;
  background:var(--ico-bg,#f1f5f9);font-size:22px;line-height:1;
  margin-bottom:14px;
  box-shadow:0 1px 4px rgba(0,0,0,.06);
}
.card-title{
  font-size:19px;font-weight:900;color:#0b1929;
  letter-spacing:-.5px;margin:0 0 8px;line-height:1.2;
}
.card-desc{
  font-size:13px;color:#475569;line-height:1.65;
  margin:0 0 20px;flex-grow:0;
}

/* ── PRODUCT WORLD SECTION inside card ─────────────────── */
.pw-section{margin-top:auto;padding-top:16px;border-top:1px solid #f1f5f9}
.pw-label{
  font-size:11px;font-weight:700;color:#0b1929;
  margin-bottom:10px;direction:rtl;letter-spacing:.1px;
}
.pw-grid{
...
.pw-btn{
  display:flex;align-items:center;justify-content:center;
  padding:8px 10px;border-radius:9px;font-size:11.5px;font-weight:600;
...
.pw-btn:hover{
  ...
  box-shadow:0 2px 8px rgba(37,99,235,.1);transform:translateY(-1px);
}

/* ── QUICK ACTIONS ──────────────────────────────────────── */
.qa-row{display:flex;gap:9px;flex-wrap:wrap;direction:rtl;justify-content:flex-end}
.qa-item{
...
/* ── DIVIDER ────────────────────────────────────────────── */
.lp-sep{border:none;height:1px;background:#f0f4f8;margin:20px 0}

/* ── FLOW / PROGRESS BAR ─────────────────────────────────── */
.flow-wrap{
  padding:24px 0 20px;direction:rtl;text-align:right;
}
.flow-eyebrow{
  font-size:10px;font-weight:800;letter-spacing:2.5px;color:#2563eb;
  text-transform:uppercase;margin-bottom:8px;
}
.flow-title{
  font-size:22px;font-weight:900;color:#0b1929;letter-spacing:-0.6px;margin:0 0 4px;
}
.flow-subtitle{
...
.flow-step:not(:last-child)::before{
  ...
  background:var(--line-c,#e2e8f0);
  z-index:0;
}
.flow-step-node{
  width:44px;height:44px;border-radius:50%;
  display:flex;align-items:center;justify-content:center;
  font-size:16px;font-weight:800;z-index:1;
  position:relative;
  background:var(--node-bg,#f1f5f9);
  border:2.5px solid var(--node-brd,#e2e8f0);
  color:var(--node-c,#94a3b8);
  transition:all .2s;
}
.flow-step.done .flow-step-node{
  background:#dbeafe;border-color:#2563eb;color:#2563eb;
  box-shadow:0 0 0 4px rgba(37,99,235,.08);
}
.flow-step.done::before{ background:#2563eb !important; }
.flow-step.active .flow-step-node{
  background:#0f2d5e;border-color:#0f2d5e;color:#fff;
  box-shadow:0 4px 16px rgba(15,45,94,.25);
}
.flow-step-label{
  margin-top:10px;font-size:12px;font-weight:700;
  color:var(--lbl-c,#94a3b8);line-height:1.35;max-width:90px;
}
.flow-step.done  .flow-step-label{ color:#1d4ed8; }
.flow-step.active .flow-step-label{ color:#0b1929; }

/* progress bar (compact, for workflow screens) */
.pb-wrap{
  display:flex;align-items:center;gap:8px;
  direction:rtl;padding:10px 0 6px;
  border-bottom:1px solid #f0f4f8;margin-bottom:14px;
}
.pb-step{
  display:inline-flex;align-items:center;gap:6px;
...
.pb-step.active{
  color:#fff;background:#0f2d5e;border-color:#0f2d5e;
}
.pb-arrow{font-size:10px;color:#cbd5e1}

/* ISA side-tool positioning */
.isa-side-label{
  font-size:10px;font-weight:700;letter-spacing:1.5px;color:#64748b;
  text-transform:uppercase;margin-bottom:6px;direction:rtl;
  display:flex;align-items:center;gap:6px;
}
.isa-side-label::before{
...
/* ── DISCLAIMER ─────────────────────────────────────────── */
.disc{
  background:#fffceb;border:1px solid #fde68a;
  border-right:3px solid #f59e0b;border-radius:10px;
  padding:8px 14px;direction:rtl;text-align:right;
  display:flex;align-items:center;gap:12px;flex-wrap:wrap;
}
.disc-lbl{font-size:10.5px;font-weight:700;color:#92400e;white-space:nowrap}
.disc-items{display:flex;gap:6px;flex-wrap:wrap}
.disc-item{
  font-size:9.5px;color:#78350f;padding:2px 9px;border-radius:999px;
  background:rgba(245,158,11,.07);border:1px solid rgba(245,158,11,.18);
}

/* ── STREAMLIT THEME OVERRIDES ──────────────────────────── */
/* Page background: very light blue-gray (not plain white) */
.stApp,[data-testid="stAppViewContainer"],[data-testid="stAppViewBlockContainer"]{
  background-color:#f2f6fb !important;
}
/* Remove red from primary CTA — use dark navy */
...
.stButton > button[kind="primary"]:hover{
  ...
  border-color:#1a3f7e !important;
}
.stButton > button[kind="primary"]:active{
  background-color:#0a1f40 !important;
  border-color:#0a1f40 !important;
}
/* Small pill style for secondary product-world buttons */
...
.gd-tip{
  background:#eff6ff;border-right:4px solid #2563eb;
  border-radius:0 10px 10px 0;padding:10px 14px;
  font-size:12px;color:#1e3a8a;margin-top:14px;line-height:1.55;
}
</style>
"""
...
def _journey_status() -> dict:
    """Return current journey step statuses."""
    ss = st.session_state
    done_pf   = bool(ss.get("journey_portfolio_done"))
    done_opt  = bool(ss.get("journey_optimization_done"))
    done_ai   = bool(ss.get("journey_ai_review_done"))
    done_rep  = bool(ss.get("journey_report_ready"))
    return {
        1: {"done": done_pf,  "label": "ניתוח תיק קיים",        "icon": "✓" if done_pf  else "1"},
        2: {"done": done_opt, "label": "אופטימיזציית תמהיל",    "icon": "✓" if done_opt else "2"},
        3: {"done": done_ai,  "label": "הסברי AI ואישור",        "icon": "✓" if done_ai  else "3"},
        4: {"done": done_rep, "label": 'דו"ח סופי ללקוח',       "icon": "✓" if done_rep else "4"},
    }

def _render_journey_flow(show_ctas: bool = True) -> None:
    """Render the 4-step visual process on the homepage."""
    steps = _journey_status()
    # Determine active step (first non-done)
    active = next((k for k in [1,2,3,4] if not steps[k]["done"]), 4)

    st.markdown("""
<div class="flow-wrap">
  <div class="flow-eyebrow">המסלול המומלץ</div>
  <div class="flow-title">כך עובד התהליך</div>
  <p class="flow-subtitle">ניתוח תיק ← תכנון ← הסברי AI ← הפקת דו״ח</p>
</div>""", unsafe_allow_html=True)

    cols = st.columns(4, gap="small")
    step_meta = [
        (1, "📊", "ניתוח תיק קיים",      "#1F4ED8"),
        (2, "🎯", "אופטימיזציית תמהיל",  "#1F4ED8"),
        (3, "🤖", "הסברי AI ואישור",      "#1F4ED8"),
        (4, "📄", 'דו"ח סופי ללקוח',     "#1F4ED8"),
    ]
    for col, (n, ico, lbl, _clr) in zip(cols, step_meta):
        _st = steps[n]
        _class = "done" if _st["done"] else ("active" if n == active else "")
        _node_content = "✓" if _st["done"] else ico
        col.markdown(f"""
<div class="flow-step {_class}" dir="rtl">
  <div class="flow-step-node">{_node_content}</div>
  <div class="flow-step-label">{lbl}</div>
</div>""", unsafe_allow_html=True)

    if show_ctas:
        st.markdown("<div style='height:14px'></div>", unsafe_allow_html=True)
        # Show a single CTA button for the next recommended step
        cta_map = {
            1: ("📊 התחל ניתוח תיק", "app", "תיק לקוח"),
            2: ("🎯 בנה תמהיל", "app", "קרנות השתלמות"),
            3: None,   # handled inside final report UI
            4: None,
        }
        cta = cta_map.get(active)
        if cta:
            label, page, pt = cta
            btn_col, _ = st.columns([1.5, 4.5])
            with btn_col:
                if st.button(label, key="journey_cta_btn", use_container_width=True, type="primary"):
                    _nav_to(page, pt)
                    st.rerun()


def _render_progress_bar(active_step: int) -> None:
    """Compact persistent progress bar for workflow screens."""
    steps = _journey_status()
    parts = []
    step_labels = {1: "ניתוח תיק", 2: "אופטימיזציה", 3: "הסברי AI", 4: 'דו"ח סופי'}
    for n in [1, 2, 3, 4]:
        _done  = steps[n]["done"]
        _cls   = "done" if _done else ("active" if n == active_step else "")
        _ico   = "✓&nbsp;" if _done else ""
        parts.append(f"<span class='pb-step {_cls}'>{_ico}{step_labels[n]}</span>")
        if n < 4:
            parts.append("<span class='pb-arrow'>›</span>")

    st.markdown(
        "<div class='pb-wrap'>" + "".join(parts) + "</div>",
        unsafe_allow_html=True,
    )


def _render_topbar(active: str = "home"):
    """Render the compact premium topbar."""
    st.markdown("""
<div class='tb'>
  <div class='tb-left'>
  ...
  <div class='tb-right'>
    <span class='tb-tagline'>Investment Intelligence Platform</span>
    <div class='tb-divider'></div>
    <span class='tb-beta'>BETA</span>
  </div>
</div>""", unsafe_allow_html=True)
    # Minimal nav: Home + Guide only (no duplicate "ניתוח תיק" — it's a card on homepage)
    nb1, nb3, nb_sp = st.columns([1, 1.1, 8])
    with nb1:
        if st.button("🏠 בית", key=f"nav_home_{active}", use_container_width=True,
                     type="primary" if active == "home" else "secondary"):
            st.session_state["app_page"] = "home"
            st.rerun()
    with nb3:
        if st.button("📖 מדריך", key=f"nav_guide_{active}", use_container_width=True,
...
def render_landing_page(df_long: "pd.DataFrame"):
    """Premium one-screen homepage. Ends with st.stop()."""
    st.markdown(_LANDING_CSS, unsafe_allow_html=True)
    _render_topbar(active="home")

    # ── 4-STEP JOURNEY FLOW ──────────────────────────────────────────
    _render_journey_flow(show_ctas=True)

    st.markdown("<hr class='lp-sep'>", unsafe_allow_html=True)

    # ── SECTION: Enter a workflow directly ──────────────────────────
    st.markdown("""
<div class="pt" style="padding-top:18px;padding-bottom:4px">
  <div class="pt-h" style="font-size:22px">כניסה מהירה לכל מסלול</div>
  <p class="pt-s">בחר מסלול לפי הצורך — ניתן לעבוד בכל מסלול בנפרד</p>
</div>""", unsafe_allow_html=True)
...
<div class="card card-primary"
     style="--rule:#2563eb;--badge-c:#2563eb;--ico-bg:#dbeafe">
  <div class="card-rule"></div>
  <div class="card-badge">&#x25B6;&nbsp; שלב 1 במסלול</div>
  <div class="card-ico">&#x1F4CA;</div>
  <div class="card-title">ניתוח תיק לקוח</div>
  <div class="card-desc">קבל תמונה מלאה על התיק לפני קבלת החלטות</div>
</div>""", unsafe_allow_html=True)
        st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)
        if st.button("התחל ניתוח תיק", key="lp_c1",
    ...
    with c2:
        ...
        st.markdown("""
<div class="card"
     style="--rule:#475569;--ico-bg:#f1f5f9">
  <div class="card-rule"></div>
  <div class="card-ico">&#x1F3AF;</div>
  <div class="card-title">אופטימיזציית תמהיל</div>
  <div class="card-desc">בנה תמהיל מותאם לפי יעדים, שארפ ושירות</div>
  <div class="pw-section">
    <div class="pw-label">בחר עולם מוצר:</div>
  </div>
</div>""", unsafe_allow_html=True)
        # 3+2 row layout — small pill buttons, each navigates immediately
        _oa, _ob, _oc = st.columns(3, gap="small")
        for _col, (_world, _label) in zip([_oa, _ob, _oc], _OPT_WORLDS[:3]):
            with _col:
                if st.button(_label, key=f"lp_opt_{_world}",
                             use_container_width=True, type="secondary"):
                    _nav_to("app", _world)
                    st.rerun()
        _od, _oe, _of = st.columns(3, gap="small")
        for _col, (_world, _label) in zip([_od, _oe], _OPT_WORLDS[3:]):
            with _col:
                if st.button(_label, key=f"lp_opt_{_world}",
                             use_container_width=True, type="secondary"):
                    _nav_to("app", _world)
    ...
    st.markdown("<hr class='lp-sep' style='margin:14px 0'>", unsafe_allow_html=True)

    # ── ISA: side tool ────────────────────────────────────────────────
    with st.expander("📐 כלי עזר — ניתוח אסטרטגיות מוסדיים (ניתוח משלים)", expanded=False):
        st.markdown("""
<div class="isa-side-label">למד מה הגופים הגדולים עשו לאורך זמן — כלי עזר לקבלת החלטות</div>""",
            unsafe_allow_html=True)
        st.caption("ניתוח זה משלים את תהליך האופטימיזציה — אינו חלק מהמסלול הראשי.")
        _ISA_WORLDS = [
            ("קרנות השתלמות", "קרנות השתלמות"),
            ("קופות גמל",      "קופות גמל"),
            ("פוליסות חיסכון", "פוליסות חיסכון"),
        ...
        _ia, _ib, _ic = st.columns(3, gap="small")
        for _col, (_world, _label) in zip([_ia, _ib, _ic], _ISA_WORLDS[:3]):
            with _col:
                if st.button(_label, key=f"lp_isa_{_world}",
                             use_container_width=True, type="secondary"):
                    _nav_to("app", _world)
                    st.rerun()
        _id, _ie, _if = st.columns(3, gap="small")
        for _col, (_world, _label) in zip([_id, _ie], _ISA_WORLDS[3:]):
            with _col:
                if st.button(_label, key=f"lp_isa_{_world}",
                             use_container_width=True, type="secondary"):
                    _nav_to("app", _world)
    ...
    st.markdown("""
<div class="qa-row">
  <span class="qa-item">&#x1F50D;&nbsp; בדוק איך התיק שלך בנוי היום</span>
  <span class="qa-item">&#x1F9E0;&nbsp; גלה מה המוסדיים עשו לאחרונה</span>
  <span class="qa-item">&#x1F916;&nbsp; שאל את ה-AI מי עדיף עכשיו</span>
  <span class="qa-item">&#x1F4CA;&nbsp; הפק דוח לתיק קיים</span>
  <span class="qa-item">&#x1F3AF;&nbsp; בנה תמהיל במהירות</span>
</div>""", unsafe_allow_html=True)
    ...
    st.markdown("""
<div class="disc">
  <span class="disc-lbl">&#x26A0;&#xFE0F;&nbsp; גרסת בטא</span>
  <div class="disc-items">
    <span class="disc-item">ייתכנו שגיאות חישוב</span>
    <span class="disc-item">נתונים עלולים להיות חלקיים</span>
    <span class="disc-item">לא ייעוץ פיננסי</span>
    <span class="disc-item">יש לאמת מול מקורות רשמיים</span>
  </div>
</div>
""", unsafe_allow_html=True)
...
# ── ROUTING: landing / guide / app ──
_page_now = st.session_state.get("app_page", "home")
if _page_now == "home":
    render_landing_page(df_long)
    # render_landing_page calls st.stop() — nothing below executes
elif _page_now == "guide":
    render_guide_page()
    # render_guide_page calls st.stop()

# ── BACK TO HOME (shown on all non-landing pages) ──
_home_col, _ = st.columns([1, 8])
with _home_col:
    if st.button("← דף הבית", key="btn_back_home", help="חזור למסך הראשי"):
        st.session_state["app_page"] = "home"
        st.rerun()
...
if _product_type_now == "תיק לקוח":
    _render_progress_bar(active_step=1)
    # Mark step 1 in progress when portfolio tab is open
    st.session_state["journey_portfolio_done"] = bool(
        st.session_state.get("portfolio_holdings")
    )
    try:
    ...
    # ── STEP 1 COMPLETE: mark and show next-step CTA ──────────────────────
    st.session_state["journey_portfolio_done"] = True
    st.markdown("---")
    st.markdown("""
<div style="direction:rtl;text-align:right;margin-bottom:12px">
  ...
  <div style="font-size:18px;font-weight:900;color:#0b1929;margin-bottom:6px">
    המשך לבניית תמהיל
  </div>
  <div style="font-size:13px;color:#475569">
    בחר עולם מוצר ובנה תמהיל מיטבי לפי יעדים ומגבלות.
  </div>
</div>""", unsafe_allow_html=True)

    _cta_l, _cta_m, _cta_r = st.columns([1.6, 1.6, 2], gap="medium")
    _opt_pairs = [
        ("קרנות השתלמות", _cta_l),
        ("קופות גמל",      _cta_m),
        ("פוליסות חיסכון", _cta_r),
    ]
    for _w, _col in _opt_pairs:
        with _col:
            if st.button(f"🎯 {_w}", key=f"cp_next_{_w}",
                         use_container_width=True, type="primary"):
                _nav_to("app", _w)
                st.rerun()

    _cta_a, _cta_b, _cta_skip = st.columns([1.2, 1.2, 3], gap="medium")
    for _w, _col in [("קרנות פנסיה", _cta_a), ("גמל להשקעה", _cta_b)]:
        with _col:
            if st.button(f"🎯 {_w}", key=f"cp_next_{_w}",
                         use_container_width=True, type="secondary"):
                _nav_to("app", _w)
                st.rerun()
    with _cta_skip:
        if st.button('📄 דלג לדו"ח — הפק על בסיס המצב הנוכחי',
                     key="cp_skip_to_report",
                     use_container_width=True, type="secondary"):
            st.session_state["journey_skip_to_report"] = True
            _nav_to("app", "קרנות השתלמות")
            st.rerun()
    st.stop()

# ── INSTITUTIONAL STRATEGY ANALYSIS (top of each tab) ──
try:
...
# ── PROGRESS BAR (optimization / ISA screens) ──
_render_progress_bar(active_step=2)
# Mark step 2 done if results exist
if st.session_state.get("last_results"):
    st.session_state["journey_optimization_done"] = True
...
if res and recs:
    ...
    # ── STEP 2 COMPLETE: mark and show CTA to report ────────────────────
    st.session_state["journey_optimization_done"] = True
    st.markdown("---")

    _step3_l, _step3_r = st.columns([2, 2], gap="large")
    with _step3_l:
        st.markdown("""
<div style="background:linear-gradient(135deg,#f0fdf4,#d1fae5);
     border:1.5px solid #6ee7b7;border-radius:14px;
     padding:18px 20px;direction:rtl;text-align:right">
  <div style="font-size:11px;font-weight:700;color:#065f46;
       letter-spacing:2px;text-transform:uppercase;margin-bottom:4px">שלב הבא</div>
  <div style="font-size:18px;font-weight:900;color:#0b1929;margin-bottom:4px">
    המשך להפקת דו"ח ללקוח
  </div>
  <div style="font-size:12.5px;color:#065f46;line-height:1.6">
    הסבר AI · עריכה · אישור · ייצוא Notebook
  </div>
</div>""", unsafe_allow_html=True)
        st.markdown("<div style='height:6px'></div>", unsafe_allow_html=True)
        if st.button('📋 המשך להפקת דו"ח', key="cta_continue_report",
                     use_container_width=True, type="primary"):
            st.session_state["_report_open"] = True
            st.rerun()
    with _step3_r:
        st.markdown("""
<div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:14px;
  ...
  <div style="font-size:11px;font-weight:700;color:#64748b;
       letter-spacing:1.5px;text-transform:uppercase;margin-bottom:4px">ניתן גם לדלג</div>
  <div style="font-size:12.5px;color:#334155;line-height:1.6">
    לא נדרש אישור AI? ייצא את תוצאות האופטימיזציה לאקסל ישירות (כפתור למעלה).
  </div>
</div>""", unsafe_allow_html=True)

    # ── STEP 3: FINAL CLIENT REPORT — always visible ─────────────────────
    st.markdown("<div style='height:12px'></div>", unsafe_allow_html=True)
    # Progress flags
    if st.session_state.get("final_report_sections"):
        st.session_state["journey_ai_review_done"] = True
    if (st.session_state.get("final_report_sections") and
            any(st.session_state.get(f"final_sec_{k}") for k in
                ["executive_summary", "final_summary"])):
        st.session_state["journey_report_ready"] = True

    st.markdown("""
<div style="background:linear-gradient(135deg,#1e293b,#0f172a);
     border-radius:14px;padding:14px 20px;direction:rtl;text-align:right;
     display:flex;align-items:center;gap:12px;margin-bottom:10px">
  <span style="font-size:24px">📋</span>
  <div>
    <div style="font-size:15px;font-weight:900;color:#f1f5f9">
      שלב 3 — דו"ח לקוח סופי
    </div>
    <div style="font-size:11.5px;color:#94a3b8">
      הסברי AI · עריכה ואישור · ייצוא Notebook
    </div>
  </div>
</div>""", unsafe_allow_html=True)

    try:
        from reporting.final_report import render_final_report_ui
        render_final_report_ui(
            rows_list    = rows_list,
            recs         = recs,
            baseline     = baseline,
            product_type = _product_type_now,
        ...
        st.warning(f"⚠️ מודול דוח לקוח לא זמין: {_fr_err}")
...
# ── INSTITUTIONAL STRATEGY ANALYSIS — rendered at top (see above) ──
pass
